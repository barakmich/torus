...
func (t *Client) ChangeDirMetadata(p torus.Path, md *models.Metadata) error {
	if !p.IsDir() {
		return torus.ErrNotDir
	}
...
func (t *Client) Rmdir(p torus.Path) error {
	if p.Path == "/" {
...
func (t *Client) SetFileEntry(p torus.Path, ent *models.FileEntry) error {
	t.srv.mut.Lock()
	defer t.srv.mut.Unlock()
	var (
		tx = t.srv.tree.Txn()
...
func (t *Client) GetChainINode(base torus.INodeRef) (torus.INodeRef, error) {
	t.srv.mut.Lock()
	defer t.srv.mut.Unlock()
	return t.srv.chains[base.Volume()][base], nil
}

func (t *Client) SetChainINode(base torus.INodeRef, was torus.INodeRef, new torus.INodeRef) error {
	t.srv.mut.Lock()
	defer t.srv.mut.Unlock()
...
func (t *Client) Getdir(p torus.Path) (*models.Directory, []torus.Path, error) {
	var (
		tx = t.srv.tree.Txn()
		k  = []byte(p.Key())
	)
	...
	tx.Root().WalkPrefix(prefix, func(k []byte, v interface{}) bool {
		subdirs = append(subdirs, torus.Path{
			Volume: p.Volume,
...
func (t *Client) ClaimVolumeINodes(_ int64, vol torus.VolumeID, inodes *roaring.Bitmap) error {
	t.srv.mut.Lock()
	defer t.srv.mut.Unlock()
...
func (t *Client) ModifyDeadMap(vol torus.VolumeID, live *roaring.Bitmap, dead *roaring.Bitmap) error {
	t.srv.mut.Lock()
	defer t.srv.mut.Unlock()
...
func (t *Client) GetVolumeLiveness(vol torus.VolumeID) (*roaring.Bitmap, []*roaring.Bitmap, error) {
	t.srv.mut.Lock()
	defer t.srv.mut.Unlock()
	x, ok := t.srv.deadMap[vol]
	if !ok {
...
func (t *Client) GetINodeChains(vid torus.VolumeID) ([]*models.FileChainSet, error) {
