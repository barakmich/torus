...
var clog = capnslog.NewPackageLogger("github.com/coreos/torus", "ninep")
...
func ListenAndServe(addr string, sv torus.FSServer) error {
	fs, err := New(sv)
	if err != nil {
		return err
	}
...
func New(sv torus.FSServer) (*AgroFS, error) {
	fs := &AgroFS{
		Srv: srv.Srv{
			// use the os users for now, torus has no user db.
			Upool: p.OsUsers,
...
func (fs *AgroFS) mkqid(apath torus.Path, d os.FileInfo) *p.Qid {
	clog.Tracef("mkqid path %s fileinfo %#v", apath, d)

	var qid p.Qid

	if d != nil {
		torusfi := d.Sys().(server.FileInfo)
		qid.Version = uint32(d.ModTime().UnixNano() / 1000000)

		if !apath.IsDir() {
			qid.Path = uint64(torusfi.Ref.INode)
		}
	}
...
func (fs *AgroFS) mkmode(apath torus.Path, d os.FileInfo) uint32 {
...
func (fs *AgroFS) Open(req *srv.Req) {
	...
	// TODO: eventually perm check will be implemented by torus for files
...
func (fs *AgroFS) packdir(apath torus.Path, owner string) (*p.Dir, error) {
	dir := new(p.Dir)
	...
	if !apath.IsDir() {
		...
		// TODO: use real mtime once torus fixes it
...
func (fs *AgroFS) Wstat(req *srv.Req) {
	...
	if dir.ChangeName() {
		...
		oldname := fid.Path
		newname := torus.Path{Volume: oldname.Volume, Path: path.Join(path.Dir(oldname.Path), dir.Name)}
